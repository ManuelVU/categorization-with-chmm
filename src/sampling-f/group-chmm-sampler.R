# CHMM of categorization sampling algorithm for group alpha and beta parameters
# requires other versions of hamiltonian functions

chmm_sampling_group <- function(data_chmm,
  ...
  sample_epsilon <- matrix(data = parameters_initial_values$epsilon, 
                           ...
                           ncol = participants, 
                           nrow = n_iterations)
  sample_alpha <- append(x = parameters_initial_values$alpha,
                         values = rep(x = NA, times = n_iterations - 1))
  sample_beta <- append(x = parameters_initial_values$beta,
                        values = rep(x = NA, times = n_iterations - 1))
  ...
  parameters_initial_values$states <- initial_states(
    ...
    inertia_category_a = rep(x = sample_alpha[1], times = participants),
    inertia_category_b = rep(x = sample_beta[1], times = participants))
  ...
  n_leaps <- 30
  acc <- append(x = 1, values = rep(x = NA, times = 99))
  ...
  for (sample in 2:n_iterations) {
    ...
    st <- foreach(pp = 1:participants) %dopar% {
      forward_backward_all(states_current =
                           ...
                           alpha = sample_alpha[(sample - 1)],
                           beta = sample_beta[(sample - 1)])
    }
    ...
    hm <- hamiltonian_mc_group(states = sample_states[, , , sample],
                               total_trials = trials_participant,
                               alpha_tilde = log(sample_alpha[(sample - 1)]),
                               beta_tilde = log(sample_beta[(sample - 1)]),
                               alpha_prior = c(2, 1), 
                               beta_prior = c(2, 1),
                               similarity = stimulus_similarity,
                               leap = n_leaps, 
    ...
    sample_alpha[sample] <- exp(hm[[1]][1])
    sample_beta[sample] <- exp(hm[[1]][2])
    if (sample <= n_burn) {
      acc[count] <- hm[[2]]
    }
    ...
    if ((sample %% 100) == 0 & (sample <= n_burn)) {
      step_size <- adjust_step(step_size = step_size, 
                               acceptance_prob = mean(acc, na.rm = TRUE),
