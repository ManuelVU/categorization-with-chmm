...
transition_others <- function(state_now,
                              state_after,
                              similarity,
                              current_id,
                              alpha,
  ...
  for (i in 1:n_others) {
    if (state_now[i] == 0 & state_after[i] == 0) {
      ...
      transition_prob[1] <- transition_prob[1] *
        logit(x = alpha - kappa * relative_similarity[i, 1]) 
      
      transition_prob[2] <- transition_prob[2] *
        logit(x = alpha - kappa * relative_similarity[i, 2])
      
    }
    else if (state_now[i] == 0 & state_after[i] == 1) {
      
      transition_prob[1] <- transition_prob[1] * 
        (1 - logit(x = alpha - kappa * relative_similarity[i, 1]))
      
      transition_prob[2] <- transition_prob[2] *
        (1 - logit(x = alpha - kappa * relative_similarity[i, 2]))
      
    }
    else if (state_now[i] == 1 & state_after[i] == 0) {
      
      transition_prob[1] <- transition_prob[1] * 
        (1 - logit(alpha + kappa * relative_similarity[i, 1]))
      
      transition_prob[2] <- transition_prob[2] * 
        (1 - logit(alpha + kappa * relative_similarity[i, 2]))
      
    }
    else {
      
      transition_prob[1] <- transition_prob[1] *
        logit(alpha + kappa * relative_similarity[i, 1])
      
      transition_prob[2] <- transition_prob[2] * 
        logit(alpha + kappa * relative_similarity[i, 2])
