...
chmm_sampling <- function(data_chmm,
                          ...
                          hmc_acceptance, 
                          prior_parameters, 
                          method_state_similarity = "average", 
                          generizability = 1) {
  ...
  if ("similarity" %in% names(data_chmm)) {
    ...
    stimulus_similarity <- similarity_ij(decay_rate = generizability, 
                                         decay_function = 1, 
  ...
  parameters_initial_values$states <- initial_states(
    ...
    inertia = sample_alpha[1, ],
    similarity_weight = sample_kappa[1, ],
    total_trials = dim(responses)[2], 
    method_state_sim = method_state_similarity)
  ...
  for (sample in 2:n_iterations) {
    ...
    for (pp in 1:participants) {
      ...
      sample_kappa[sample, pp] <- exp(hm[[pp]][[1]][2])
      if (sample <= n_burn){
