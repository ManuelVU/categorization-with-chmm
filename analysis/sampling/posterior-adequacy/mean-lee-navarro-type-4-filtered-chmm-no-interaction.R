# Posterior mean responses no interactions CHMM approach in Lee and Navarro 2002
...
source(file = "src/sampling-f/no-interaction/posterior-adequacy-sampler.R")
...
for (p in participants_keep){
  ...
  posterior_add[count, 1:participant_trials] <- colMeans(y_hat_tmp)
  
}

saveRDS(object = posterior_add,
        file = paste(c("data/posterior-samples/posterior-adequacy/",
                       "mean-lee-navarro-type4-filtered-chmm-no-interaction.rds"), 
                     collapse = ""))
